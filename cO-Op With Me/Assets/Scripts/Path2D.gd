#Script do caminho do player na introção 
extends Path2D
...
func _process(delta):
	...
	#Condição para Player realizar animação de andando para o lado se o offset for entre 180 e 370
	elif $PathFollow2D.offset >= 180 and $PathFollow2D.offset < 370:
		$PathFollow2D/Player_indro/AnimatedSprite.play("run_side")
	#Condição para Player realizar animação de parado de frente se o offset for maior ou igual a 370
