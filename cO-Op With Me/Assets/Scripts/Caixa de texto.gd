...
func _process(delta):
	...
	if Input.is_action_just_pressed("action"):
		contador += 1
		Global.chat = 1
	#Botão para mudar o contador2 depois de ter colocado o bolo na bancada
	...
	if contador >= 7:
		#Passando o número 7 no contador tudo fica invisível
		$Chat/Resposta_Atendente.visible = false
		$Chat/Kaled_Atendente.visible = false
		visible = false
		$Chat/Pedido2.visible = false
	...
	if Global.tem_bolo_assado == 0 and Global.pegou_bolo_assado == 0:
		...
		$ColorRect.visible = true
	if contador2 == 2:
		#Fala do Otto fica visível e sua imagem
	...
	if contador2 == 4:
		...
		$Chat/Player_cozinheiro.visible = false
	if contador2 >= 5:
		#Deixa tudo invisível depois de 5 fases
		visible = false
